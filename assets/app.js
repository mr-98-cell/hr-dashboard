...
(function () {
  ...
  /* ---------------- الوظائف: سجلّ مرجعي مستقل ----------------
     كل الوظائف بفلاتر الإدارة والمرحلة ونوع التوظيف، وعرضين:
     بطاقات تُظهر مسار المراحل، وجدول للمسح السريع. */
  function pageJobs() {
    const all = rows("jobs");
    const byType = state.jobType ? all.filter((j) => (j.hire_type || "") === state.jobType) : all;
    const atStage = (i) => byType.filter((j) => Number(j.stage) === i);
    const shown = state.stage == null ? byType : atStage(state.stage);

    /* شريط الفلاتر — «الإدارة» يكتب في الفلتر العام نفسه فلا يتضارب مصدران */
    ...
    const filters = `<div class="jfilters">
      <label class="tb sel-wrap"><span class="k">${esc(t("الإدارة"))}</span>
        <select onchange="APP.setFilter('unit',this.value)">${unitOpts}</select>${CHEV}</label>
      <label class="tb sel-wrap"><span class="k">${esc(t("المرحلة"))}</span>
        <select onchange="APP.setStage(this.value===''?null:Number(this.value),true)">${stageOpts}</select>${CHEV}</label>
      <label class="tb sel-wrap"><span class="k">${esc(t("نوع التوظيف"))}</span>
        <select onchange="APP.setJobType(this.value)">${typeOpts}</select>${CHEV}</label>
      <div class="sp"></div>
      <span class="vswitch">
        <b class="${view === "cards" ? "on" : ""}" onclick="APP.setJobsView('cards')">${esc(t("بطاقات"))}</b>
        <b class="${view === "table" ? "on" : ""}" onclick="APP.setJobsView('table')">${esc(t("جدول"))}</b></span>
      ${canEdit() ? `<button class="btn btn-p" onclick="APP.openForm('job')">${icon("plus")} ${esc(t("إضافة وظيفة"))}</button>` : ""}
    </div>`;
    ...
    const tiles = `<div class="sumrow" style="grid-template-columns:repeat(${STAGES.length},1fr)">${sum}</div>`;
    ...
    const cards = shown.map((j, i) => {
      ...
      return `<div class="acc ${i === 0 ? "open" : ""}"><div class="head" onclick="this.parentElement.classList.toggle('open')">
        ...
        <span class="badge ${j.hire_type === "بديلة" ? "b-warn" : "b-info"}">${esc(t(STAGES[j.stage] || ""))}</span><span class="chev">${CHEV}</span></div>
    ...
    const head = ["رقم الوظيفة", "المسمى الوظيفي", "الإدارة", "الدرجة الوظيفية", "نوع التوظيف", "المرحلة", "المرشح", "المتقدمون", "تاريخ المباشرة"];
    const body = shown.map((j) => {
      const act = canEdit()
        ? `<span class="iact" title="${esc(t("تعديل"))}" onclick="APP.openForm('job','${jsq(j.id)}')">${icon("pen")}</span>
           <span class="iact del" title="${esc(t("حذف"))}" onclick="APP.removeRow('jobs','${jsq(j.id)}')">${icon("trash")}</span>` : "";
      return `<tr><td class="mono">${esc(j.code || "—")}</td><td>${esc(j.title || "—")}</td>
        <td>${esc(uname(unitById(j.unit_id)) || "—")}</td><td>${esc(j.grade || "—")}</td>
        <td>${esc(j.hire_type || "—")}</td>
        <td><span class="badge ${Number(j.stage) === JOINED ? "b-good" : Number(j.stage) === 0 ? "b-warn" : "b-info"}">${esc(t(STAGES[j.stage] || ""))}</span></td>
        <td>${esc(j.candidate || "—")}</td><td>${esc(String(j.received || 0))}</td>
        <td>${esc(j.start_date ? fmtDate(j.start_date) : "—")}</td><td class="ta">${act}</td></tr>`;
    }).join("");
    const table = `<div class="tblwrap"><table class="dtable"><thead><tr>${
      head.map((h) => `<th>${esc(t(h))}</th>`).join("")}<th></th></tr></thead><tbody>${body}</tbody></table></div>`;

    const note = state.stage == null && !state.jobType ? "" :
      `<div class="stagenote">${esc(t("تعرض الآن"))} ${shown.length} ${esc(t("وظيفة"))}
        <button class="btn btn-x" onclick="APP.clearJobFilters()">${esc(t("إلغاء الفلاتر"))}</button></div>`;

    const body2 = shown.length
  ...
  function shell(title, body) {
    const nav = [["index", "الرئيسية", "home"], ["recruitment", "التوظيف", "users"], ["jobs", "الوظائف", "case"], ["training", "التدريب", "cap"],
...
${editBtn}
  ...
  const STR = {
    ...
    "مفتوحة": "Open", "وظيفة": "positions", "تعرض الآن": "Showing",
    "إلغاء الفلاتر": "Clear filters", "بلا مرشح بعد": "No candidate yet",
    "لا توجد وظائف مطابقة للفلاتر الحالية": "No positions match the current filters", "منتهي": "Ended", "عرض الكل": "Show all", "بديل استقالة": "Backfill for", "وحدات": "units", "نسبة الإشغال مقابل الوظائف": "Occupancy vs positions", "الوظائف والانضمام": "Positions and onboarding", "انضم": "joined", "كل الأشهر": "All months", "تعديل العناوين": "Edit titles",
    // الصلاحيات
  ...
  const PAGES = {
    ...
    jobs: { title: "الوظائف", render: pageJobs },
