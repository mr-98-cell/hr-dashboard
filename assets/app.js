...
(function () {
  ...
  const ICONS = {
    ...
    upload: '<path d="M4 15v4a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-4"/><path d="M12 16V4"/><path d="m7.5 8.5 4.5-4.5 4.5 4.5"/>',
  };
  ...
  function pageExec() {
    ...
    const kpis = `<div class="kstrip">
      ...
      ${kpiTile("مرشحون تحت الإجراء", pipeline, null, t("في مراحل التوظيف"))}
...
<Font ss:Color="#FFFFFF" ss:Bold="1"/></Style></Styles>${sheets}</Workbook>`;
  ...
  /* ---------------- أرقام الوحدات: قالب واستيراد ---------------- */
  // الأرقام تُدخَل على الوحدات النهائية فقط — الوحدات الأعلى تُجمَّع تلقائيًا
  function leafUnits() { return units().filter((u) => childrenOf(u.id).length === 0); }
  const NUM_COLS = [["approved", "المعتمدة"], ["filled", "المشغولة"], ["male", "ذكور"], ["female", "إناث"]];

  /* قالب Excel بكل الوحدات النهائية وقيمها الحالية */
  function exportNumbersTemplate() {
    const xesc = (v) => String(v == null ? "" : v).replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
    const head = ["المعرّف", "الوحدة", "التبعية"].concat(NUM_COLS.map((c) => c[1]));
    const rows = leafUnits().map((u) => {
      const parent = pathOf(u.id).slice(0, -1).map((x) => (unitById(x) || {}).name).join(" ← ") || "—";
      return [u.id, u.name, parent].concat(NUM_COLS.map((c) => Number(u[c[0]] || 0)));
    });
    const xml = `<?xml version="1.0" encoding="UTF-8"?>
<?mso-application progid="Excel.Sheet"?>
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
<Styles><Style ss:ID="h"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#00584C" ss:Pattern="Solid"/></Style>
<Style ss:ID="lock"><Interior ss:Color="#EEF6F4" ss:Pattern="Solid"/></Style></Styles>
<Worksheet ss:Name="أرقام الوحدات"><Table>
<Row>${head.map((h) => `<Cell ss:StyleID="h"><Data ss:Type="String">${xesc(h)}</Data></Cell>`).join("")}</Row>
${rows.map((r) => `<Row>${r.map((v, i) => i < 3
        ? `<Cell ss:StyleID="lock"><Data ss:Type="String">${xesc(v)}</Data></Cell>`
        : `<Cell><Data ss:Type="Number">${xesc(v)}</Data></Cell>`).join("")}</Row>`).join("\n")}
</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><DisplayRightToLeft/></WorksheetOptions></Worksheet></Workbook>`;
    download(`قالب-أرقام-الوحدات-${new Date().toISOString().slice(0, 10)}.xls`, "application/vnd.ms-excel", xml);
    toast("نُزّل القالب — عبّئ الأرقام ثم احفظه CSV وارفعه");
  }

  /* تقسيم سطر CSV مع احترام علامات الاقتباس */
  ...
  /* استيراد الأرقام — يقبل الفاصلة أو الفاصلة المنقوطة أو Tab */
  async function importNumbers(file) {
    if (!canEdit()) return;
    let text = await file.text();
    text = text.replace(/^\ufeff/, "").replace(/\r\n?/g, "\n");
    const lines = text.split("\n").filter((l) => l.trim());
    if (lines.length < 2) { alert("الملف فارغ أو بلا صفوف بيانات"); return; }

    const d = [",", ";", "\t"].map((x) => [x, lines[0].split(x).length]).sort((a, b) => b[1] - a[1])[0][0];
    const head = splitCSV(lines[0], d);
    const idAt = head.indexOf("المعرّف");
    const idx = {};
    NUM_COLS.forEach(([k, label]) => { idx[k] = head.indexOf(label); });
    if (idAt < 0 || NUM_COLS.some(([k]) => idx[k] < 0)) {
      alert("أعمدة القالب غير مكتملة — استخدم القالب المنزَّل دون تغيير أسماء الأعمدة");
      return;
    }
    ...
    for (let n = 1; n < lines.length; n++) {
      const c = splitCSV(lines[n], d);
      const id = c[idAt];
      const u = unitById(id);
      if (!u) { errs.push(`سطر ${n + 1}: معرّف غير معروف «${id}»`); continue; }
      const patch = {};
      let bad = false;
      for (const [k, label] of NUM_COLS) {
        const raw = String(c[idx[k]] || "0").replace(/[^0-9.-]/g, "");
        const v = Number(raw);
        if (!isFinite(v) || v < 0 || v % 1) { errs.push(`سطر ${n + 1}: قيمة غير صالحة في «${label}»`); bad = true; break; }
        patch[k] = v;
      }
      if (bad) continue;
      if (patch.filled > patch.approved) warns.push(`${u.name}: المشغولة (${patch.filled}) أكبر من المعتمدة (${patch.approved})`);
      if (NUM_COLS.every(([k]) => Number(u[k] || 0) === patch[k])) { same++; continue; }
      try { await DB.update("org_units", id, patch); changed++; }
      catch (e) { errs.push(`${u.name}: تعذّر الحفظ — ${e.message}`); }
    }

    render();
    let msg = `حُدِّثت ${changed} وحدة · ${same} بلا تغيير`;
    if (warns.length) msg += `\n\n⚠️ ${warns.length} تنبيه:\n` + warns.slice(0, 8).join("\n");
    if (errs.length) msg += `\n\n❌ ${errs.length} خطأ:\n` + errs.slice(0, 8).join("\n");
    alert(msg);
  }
...
${editBtn}
      ...
      <input type="file" id="numFile" accept=".csv,text/csv,text/plain" style="display:none" onchange="APP.loadNumbers(this)">
    </div>`;
  }
  ...
  const STR = {
    ...
    "في مراحل التوظيف": "across the recruitment stages", "تعيينات": "hires", "استقالات": "resignations",
  ...
  window.APP = {
    ...
    async loadNumbers(el) { if (el.files && el.files[0]) await importNumbers(el.files[0]); },
